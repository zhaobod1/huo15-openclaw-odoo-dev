/**
 * 火一五·辉火云企业套件插件 (Odoo 19 Plugin) - 多用户版
 *
 * 功能：
 * 1. 自然语言操作 Odoo（创建待办、提醒、活动等）
 * 2. Odoo 通知同步到 OpenClaw
 * 3. 智能意图理解和路由
 * 4. 多用户隔离（每个用户独立的 Odoo 配置）
 *
 * 使用方式：
 * - 用户首次操作时自动引导配置（URL、数据库、账号、密码）
 * - 每个用户配置独立存储，完全隔离
 * - 配置后插件自动连接并轮询通知
 *
 * ⚠️ 安全说明：当前版本密码明文存储在本地
...
import type { OdooPluginConfig, SyncUpdate, IntentResult, OdooConfig } from './src/types/index.js';
import {
  resolveOdooAccount,
  getOdooConfigForAgent,
  isUserConfigured,
  saveOdooConfigForAgent,
  normalizeAgentId,
...
const pluginState = {
  getStatus(agentId: string = 'default') {
    ...
    return {
      ...
      configured: isUserConfigured(agentId),
    };
  },

  async connect(api: any, agentId: string, config: OdooConfig) {
    ...
    const syncConfig = pluginConfig?.sync ?? { enabled: true, intervalSeconds: 30, channels: ['todo', 'activity', 'message'] };
    ...
    console.log(`[Odoo Plugin] 已连接到 Odoo（用户: ${agentId}），UID: ${client.getUid()}`);
    return client;
  },
};
...
export default definePluginEntry({
  id: 'odoo-dev',
  name: '火一五·辉火云企业套件插件',
  description: '自然语言操作辉火云企业套件（Odoo 19）、通知同步、待办管理、活动提醒（多用户隔离版）',
  ...
  register(api) {
    ...
    console.log('[Odoo Plugin] 插件已加载（多用户隔离版）');
  },
});
 ...
 * 如果用户未配置，返回 null
 */
async function getOrInitClient(api: any, agentId: string): Promise<OdooClient | null> {
  const normalizedAgentId = normalizeAgentId(agentId);

  // 已连接的直接返回
  if (odooClients.has(normalizedAgentId)) {
    const client = odooClients.get(normalizedAgentId)!;
  ...
  // 尝试加载已保存的配置并连接
  const config = getOdooConfigForAgent(normalizedAgentId);
  if (config) {
    try {
      await pluginState.connect(api, normalizedAgentId, config);
      return odooClients.get(normalizedAgentId) ?? null;
    } catch (error) {
      console.error(`[Odoo Plugin] 恢复连接失败（${normalizedAgentId}）:`, error);
      return null;
    }
  }
 ...
 * 检查用户是否需要配置引导
 */
function checkAndPromptSetup(agentId: string): { needsSetup: boolean; message?: string } {
  const normalizedAgentId = normalizeAgentId(agentId);

  if (!checkUserConfigured(normalizedAgentId)) {
...
function registerTools(api: any) {
  ...
  api.registerTool({
    name: 'odoo_connect',
    description: '连接辉火云企业套件（Odoo 19）系统。需要提供系统地址、数据库名、账号和密码。首次使用会自动引导配置。',
    schema: {
      type: 'object',
      properties: {
        url: {
    ...
    async handler(params: { url: string; db: string; username: string; password: string }, ctx: any) {
      ...
      if (!validation.valid) {
        return { success: false, message: `❌ 连接失败：${validation.error}\n\n请检查配置是否正确后重试。` };
      }

      // 保存并连接
      const saved = saveOdooConfigForAgent(agentId, config);
      if (!saved) {
        return { success: false, message: '❌ 配置保存失败，请稍后重试。' };
      }

      try {
        await pluginState.connect(api, agentId, config);
        return { success: true, message: `✅ 已成功连接到 ${params.url}，配置已保存。` };
      } catch (error) {
  ...
  api.registerTool({
    ...
    schema: {
      type: 'object',
      properties: {
        name: { type: 'string', description: '待办名称/标题' },
    ...
    async handler(params: any, ctx: any) {
      ...
      // 检查是否已配置
      const setupCheck = checkAndPromptSetup(agentId);
      if (setupCheck.needsSetup) {
        return { success: false, message: setupCheck.message, needsSetup: true };
      }

      const client = await getOrInitClient(api, agentId);
      if (!client) {
        return { success: false, message: '❌ 连接失败，请重新配置。', needsSetup: true };
      }
  ...
  api.registerTool({
    ...
    async handler(params: any, ctx: any) {
      ...
      try {
        const tasks = await client.getMyTasks({ limit: params.limit || 50, project_id: params.project_id });
        return {
          success: true,
          tasks: tasks.map((t: any) => ({
            id: t.id, name: t.name, description: t.description,
            date_deadline: t.date_deadline, priority: t.priority, stage: t.stage_id,
          })),
          count: tasks.length,
        };
      } catch (error) {
  ...
  api.registerTool({
    ...
    schema: {
      ...
      properties: {
        ...
        stop: { type: 'string', description: '结束时间，格式 YYYY-MM-DD HH:MM:SS' },
        description: { type: 'string', description: '事件描述' },
        partner_ids: { type: 'array', items: { type: 'number' }, description: '参与人ID列表' },
      },
    ...
    async handler(params: any, ctx: any) {
      ...
      try {
        const eventId = await client.createCalendarEvent({
          name: params.name, start: params.start, stop: params.stop,
          description: params.description, partner_ids: params.partner_ids,
        });
  ...
  api.registerTool({
    ...
    schema: {
      type: 'object',
      properties: {
        res_model: { type: 'string', description: '关联模型，如 project.task' },
        res_id: { type: 'number', description: '关联记录ID' },
        activity_type_id: { type: 'number', description: '活动类型ID' },
        summary: { type: 'string', description: '活动摘要' },
        note: { type: 'string', description: '活动详细说明' },
        date_deadline: { type: 'string', description: '截止日期，格式 YYYY-MM-DD' },
        user_id: { type: 'number', description: '负责人ID' },
      },
    ...
    async handler(params: any, ctx: any) {
      ...
      try {
        const activityId = await client.createActivity({
          res_model: params.res_model, res_id: params.res_id,
          activity_type_id: params.activity_type_id, summary: params.summary,
          note: params.note, date_deadline: params.date_deadline, user_id: params.user_id,
        });
  ...
  api.registerTool({
    ...
    schema: {
      type: 'object',
      properties: {
        model: { type: 'string', description: '模型名称，如 res.partner, project.task, crm.lead' },
        domain: { type: 'array', description: '搜索域，格式 [[field, operator, value], ...]' },
    ...
    async handler(params: any, ctx: any) {
      ...
      try {
        const result = await client.searchRead(
          params.model, params.domain || [],
          params.fields || ['id', 'name'],
          { limit: params.limit || 50 }
        );
        return { success: true, records: result.records, count: result.length };
      } catch (error) {
        return { success: false, message: `查询失败: ${error}` };
      }
    },
  ...
  api.registerTool({
    ...
    schema: {
      type: 'object',
      properties: {
        model: { type: 'string', description: '目标模型' },
    ...
    async handler(params: any, ctx: any) {
      ...
      try {
        const messageId = await client.call('mail.message', 'create', [{
          model: params.model, res_id: params.res_id,
          body: params.body, message_type: 'comment',
        }]);
    ...
    async handler(_params: any, ctx: any) {
      ...
      if (!status.configured) {
        return {
          success: true,
          ...status,
          message: '🔔 尚未配置 Odoo 连接，首次使用需要先配置。',
          needsSetup: true,
        };
      }
  ...
  // 解析自然语言配置（辅助工具，供 AI 调用）
  api.registerTool({
    name: 'odoo_setup_from_text',
    description: '从自然语言中解析 Odoo 配置信息并保存',
    schema: {
      type: 'object',
      properties: {
        text: { type: 'string', description: '包含配置信息的自然语言文本' },
      },
      required: ['text'],
    },
    async handler(params: { text: string }, ctx: any) {
      const agentId = normalizeAgentId(ctx.agentId);

      // 解析配置
      const parsed = parseUserConfigInput(params.text);
      if (!parsed) {
        const { prompt } = generateSetupPrompt();
        return {
          success: false,
      ...
      // 验证必须字段
      if (!parsed.url || !parsed.db || !parsed.username || !parsed.password) {
        return {
          success: false,
          message: `❌ 配置信息不完整。请提供：\n- 系统地址（URL）\n- 数据库名称\n- 用户名\n- 密码`,
        };
      ...
      // 保存并连接
      return await setupUserOdoo(agentId, parsed as OdooConfig);
    },
  });
...
function registerHooks(api: any) {
  api.registerHook('onMessage', async (text: string, user: string, channelName: string, msg: any) => {
    ...
    // 检测是否是配置类指令
    const configKeywords = ['连接', '配置', 'setup', 'connect', 'odoo'];
    const isConfigIntent = configKeywords.some(kw => text.toLowerCase().includes(kw));

    // 如果用户尚未配置且不是配置指令，引导配置
    if (!isUserConfigured(agentId) && !isConfigIntent) {
      // 只有明确是 Odoo 操作意图时才引导
      const odooKeywords = ['待办', '日历', '客户', '商机', '订单', '项目', '任务', '提醒', 'odoo'];
      if (odooKeywords.some(kw => text.includes(kw))) {
        const { prompt } = generateSetupPrompt();
        return {
  ...
  try {
    switch (intent.intent) {
      ...
      case 'todo.create.withReminder': {
        ...
        return { content: `✅ 已创建待办「${intent.entities.name}」，ID: ${taskId}` };
      }
      ...
      case 'todo.list': {
        ...
        const list = tasks.map((t: any, i: number) =>
          `${i + 1}. ${t.name}${t.date_deadline ? ` (截止: ${t.date_deadline})` : ''}`
        ).join('\n');
        return { content: `📋 您的待办列表：\n${list}` };
      }
      ...
      case 'calendar.create.withDate': {
        ...
        return { content: `✅ 已创建日历事件「${intent.entities.eventName}」，ID: ${eventId}` };
      }
      ...
      case 'search.contact': {
        ...
        const list = result.records.map((p: any, i: number) =>
          `${i + 1}. ${p.name} ${p.phone || ''}`
        ).join('\n');
        return { content: `🔍 客户搜索结果：\n${list}` };
      }

      case 'project.list': {
        const result = await client.searchRead('project.project', [['active', '=', true]], ['id', 'name', 'partner_id'], { limit: 20 });
        if (result.records.length === 0) {
      ...
      case 'crm.opportunity.list': {
        const result = await client.searchRead('crm.lead', [['type', '=', 'opportunity'], ['active', '=', true]], ['id', 'name', 'partner_id', 'stage_id', 'probability'], { limit: 20 });
        if (result.records.length === 0) {
      ...
      case 'sale.order.list': {
        const result = await client.searchRead('sale.order', [['state', 'not in', ['cancel']]], ['id', 'name', 'partner_id', 'date_order', 'amount_total', 'state'], { limit: 20 });
        if (result.records.length === 0) {
        ...
        return { content: `📦 销售订单列表：\n${list}` };
      }

      case 'stock.inventory': {
        const term = intent.entities.term as string || '';
        const domain: any = term ? [['product_id.name', 'ilike', term]] : [];
        const result = await client.searchRead('stock.quant', domain, ['id', 'product_id', 'quantity', 'location_id'], { limit: 20 });
        if (result.records.length === 0) {
      ...
      case 'project.task.recordHours': {
        ...
        const timesheetId = await client.create('account.analytic.line', {
          name: description, unit_amount: hours,
          date: new Date().toISOString().split('T')[0],
          user_id: client.getUid() || 1,
        });
        return { content: `✅ 已记录工时 ${hours} 小时，ID: ${timesheetId}` };
      }

      case 'todo.done': {
        const taskName = intent.entities.taskName as string;
        if (!taskName) return { content: '请指定要完成的待办名称' };
        const tasks = await client.searchRead('project.task', [['name', 'ilike', taskName], ['active', '=', true]], ['id', 'name'], { limit: 1 });
        if (tasks.records.length === 0) return { content: `未找到待办：${taskName}` };
        await client.write('project.task', [tasks.records[0].id as number], { active: false });
        return { content: `✅ 已完成待办「${taskName}」` };
      }
...
function handleOdooUpdates(api: any, updates: SyncUpdate[], agentId: string) {
  for (const update of updates) {
    ...
    switch (update.type) {
      case 'todo':
        message = update.action === 'create'
          ? `📋 新待办：${(update.data as any).name}`
          : `📋 待办更新：${(update.data as any).name}`;
        break;
      case 'activity':
...
function resolveRelativeDate(relative: string | null, explicitDate: string | null, type: 'start' | 'end'): string {
  ...
  if (relative === 'tomorrow') now.setDate(now.getDate() + 1);
  else if (relative === 'day after tomorrow') now.setDate(now.getDate() + 2);
  else if (relative === 'next week') now.setDate(now.getDate() + 7);
  ...
  if (type === 'start') now.setHours(9, 0, 0, 0);
