...
import { p as writeFileFromPathWithinRoot } from "./fs-safe-B2XplAF8.js";
import { t as SsrFBlockedError } from "./ssrf-TOfbRtf4.js";
import "./text-runtime-BngW26kR.js";
import { A as withBrowserNavigationPolicy, C as getChromeWebSocketUrl, D as assertBrowserNavigationAllowed, E as InvalidBrowserNavigationUrlError, F as DEFAULT_UPLOAD_DIR, G as fetchJson, K as getHeadersWithAuth, L as resolveStrictExistingPathsWithinRoot, O as assertBrowserNavigationRedirectChainAllowed, P as DEFAULT_TRACE_DIR, S as BrowserTabNotFoundError, T as normalizeCdpWsUrl, W as appendCdpPath, X as withCdpSocket, Z as withNoProxyForCdpUrl, a as buildRoleSnapshotFromAriaSnapshot, c as matchBrowserUrlPattern, i as buildRoleSnapshotFromAiSnapshot, j as markPwAiLoaded, k as assertBrowserNavigationResultAllowed, o as getRoleSnapshotStats, q as normalizeCdpHttpBaseForJsonEndpoints, s as parseRoleRef, w as formatAriaSnapshot } from "./client-fetch-CraVgYfL.js";
...
function observeDelayedInteractionNavigation(page, previousUrl) {
	if (didCrossDocumentUrlChange(page, previousUrl)) return Promise.resolve(true);
	if (typeof page.on !== "function" || typeof page.off !== "function") return Promise.resolve(false);
...
async function assertInteractionNavigationCompletedSafely(opts) {
	...
	const onFrameNavigated = (frame) => {
		if (!isMainFrameNavigation(navPage, frame)) return;
	...
	if (navigatedDuringAction || didCrossDocumentUrlChange(opts.page, opts.previousUrl)) await assertPageNavigationCompletedSafely({
		cdpUrl: opts.cdpUrl,
		page: opts.page,
		response: null,
		ssrfPolicy: opts.ssrfPolicy,
		...
		if (await observeDelayedInteractionNavigation(opts.page, opts.previousUrl)) await assertPageNavigationCompletedSafely({
			cdpUrl: opts.cdpUrl,
			page: opts.page,
			response: null,
			ssrfPolicy: opts.ssrfPolicy,
...
async function executeSingleAction(action, cdpUrl, targetId, evaluateEnabled, ssrfPolicy, depth = 0) {
...
async function batchViaPlaywright(opts) {
	...
	for (const action of opts.actions) try {
		await executeSingleAction(action, opts.cdpUrl, opts.targetId, opts.evaluateEnabled, opts.ssrfPolicy, depth);
