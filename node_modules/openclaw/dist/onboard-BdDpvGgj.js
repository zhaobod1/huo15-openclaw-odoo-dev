...
import { f as readConfigFileSnapshot, r as replaceConfigFile } from "./config-nRyLQtXB.js";
...
import "./daemon-runtime-Ck5RMYvp.js";
import { t as resolveControlUiLinks } from "./control-ui-links-C-r_G3CC.js";
import { c as normalizeGatewayTokenInput, d as randomToken, h as waitForGatewayReachable, n as applyWizardMetadata, o as handleReset, r as ensureWorkspaceAndSessions, t as DEFAULT_WORKSPACE } from "./onboard-helpers-BjBs8qWy.js";
...
async function collectGatewayHealthFailureDiagnostics() {
	const diagnostics = {};
	try {
		const { resolveGatewayService } = await import("./service-CQdtpPlS.js");
	...
	try {
		const { readLastGatewayErrorLine } = await import("./diagnostics-BC8eDqPT.js");
...
async function runNonInteractiveLocalSetup(params) {
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-KWX3skoy.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-C87-l78u.js");
	...
	if (!opts.skipHealth) {
		const { healthCommand } = await import("./health-Byap0Kws.js");
