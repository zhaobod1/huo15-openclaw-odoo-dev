 ...
 */
function createAssistantSayStream(args) {
    const { client, context, payload } = args;
    const { channelId, threadTs } = extractThreadInfo(payload);
    return (0, context_1.createSayStream)(client, context, channelId, threadTs);
}
/**
 * Creates utility `setStatus()` to set the status and indicate active processing.
 * https://api.slack.com/methods/assistant.threads.setStatus
 */
function createAssistantSetStatus(args) {
    const { client, payload } = args;
    const { channelId, threadTs } = extractThreadInfo(payload);
    return (0, context_1.createSetStatus)(client, channelId, threadTs);
}
/**
...
function extractThreadInfo(payload) {
    let context = {};
    if ('assistant_thread' in payload && (0, helpers_1.isRecord)(payload.assistant_thread)) {
        if ((0, helpers_1.isRecord)(payload.assistant_thread.context)) {
            context = payload.assistant_thread.context;
        }
    }
    const channelId = (0, helpers_1.extractEventChannelId)(payload);
    const threadTs = (0, helpers_1.extractEventThreadTs)(payload);
    if (!channelId || !threadTs) {
        ...
        throw new errors_1.AssistantMissingPropertyError(`Assistant message event is missing required properties: ${missingProps.join(', ')}`);
    }
